...
public class CustomLocalDateTimeDeserializer extends StdDeserializer<LocalDateTime> {
  ...
  public LocalDateTime deserialize(JsonParser parser, DeserializationContext context)
        ...
        if (string.length() > LOCAL_DATE_LENGTH) {
          if (string.charAt(LOCAL_DATE_LENGTH) == 'T') {
            ...
            throw JsonMappingException.from(parser,
                String.format("Failed to deserialize %s as LocalDateTime", string));
          }
        } else {
    ...
    if (parser.isExpectedStartArrayToken()) {
      ...
      if (parser.nextToken() != JsonToken.END_ARRAY) {
        ...
        if (parser.nextToken() != JsonToken.END_ARRAY) {
          ...
          if (partialSecond < MAX_MILLIS && !context.isEnabled(
              DeserializationFeature.READ_DATE_TIMESTAMPS_AS_NANOSECONDS)) {
            partialSecond *= MILLI_TO_NANO; // value is milliseconds, convert it to nanoseconds
          }

          if (parser.nextToken() != JsonToken.END_ARRAY) {
            throw context.wrongTokenException(parser, LocalDateTime.class, JsonToken.END_ARRAY,
    ...
    throw context.wrongTokenException(parser, LocalDateTime.class, JsonToken.VALUE_STRING,
  ...
  private void rethrowDateTimeException(JsonParser p, DeserializationContext context,
    ...
    if (e0 instanceof DateTimeParseException) {
      ...
      e = JsonMappingException.from(p,
          String.format("Failed to deserialize %s: (%s) %s", handledType().getName(),
